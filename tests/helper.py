...
randstr = lambda x: ''.join(random.choice(
    string.ascii_uppercase + string.digits) for x in xrange(x))

randchars = lambda x: ''.join(random.choice(
    string.ascii_uppercase) for x in xrange(x))

randdigits = lambda x: ''.join(random.choice(
    string.digits) for x in xrange(x))


class TestingFile(StringIO):
    def close(self):
        pass
